...
const TableSubCategory = (prop:PropsInterface) => {
  ...
  const rows = data?.map((element,key) => (
    <Table.Tr key={key}>
      ...
      <Table.Td>{element.sub_category_name_vn}</Table.Td>
      <Table.Td>{element.sub_category_name_de}</Table.Td>
      <Table.Td>{element.sub_category_name_th}</Table.Td>
      <Table.Td>{element.sub_category_name_eng}</Table.Td>
      <Table.Td>{element.category_id}</Table.Td>
      <Table.Td>{`${element.dph}%`}</Table.Td>
      <Table.Td>
        <Group>
